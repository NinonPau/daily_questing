...
=======
  # Friendships (relations réciproques)
  ...
  def add_creator_as_participant
    task_participants.find_or_create_by(user: self, status: "accepted")
>>>>>>> completed
  end
...
=======
  ...
  def xp_progress_percent
    ...
    range = levels[current_level]
    return { percent: 100, remaining: 0 } unless range

    min, max = range.begin, range.end
    xp_into_level = xp - min
    xp_required = [max - min, 1].max
    percent = (xp_into_level.to_f / xp_required) * 100

    {
      percent: percent.clamp(0, 100).round(2),
